...
export function useHasTransferHookEnabled(mint: PublicKey) {
  ...
  const programId = useMemo(
    () => getABLTokenProgramId(cluster.network as Cluster),
    [cluster]
  );
...
export function useGetToken(mint: PublicKey) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const mode =
        metadata?.additionalMetadata.find(
          (metadata) => metadata[0] === "AB"
        )?.[1] || null;
      const threshold =
        metadata?.additionalMetadata.find(
          (metadata) => metadata[0] === "threshold"
        )?.[1] || null;
      ...
      const isTransferHookSet =
        transferHook?.programId?.equals(programId) || false;
...
export function useAblTokenProgram() {
  ...
  const program = useMemo(
    () => getABLTokenProgram(provider, programId),
    [provider, programId]
  );
  ...
  const processBatchWallets = useMutation({
    ...
    onError: (error: Error) => {
      ...
      toast.error(
        `Failed to run program: ${error?.message || "Unknown error"}`
      );
    },
  });
